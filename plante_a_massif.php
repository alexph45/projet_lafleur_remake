...
// Récupérer les produits de la catégorie "plante à massif" (ID = 3)
try {
    $stmt = $pdo->prepare('SELECT * FROM produits WHERE id_cat = 3');
    $stmt->execute();
    $produits_massif = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Récupérer le nom de la catégorie "plante à massif" (ID = 3)
    $cat_stmt = $pdo->prepare('SELECT nom_categorie FROM categories WHERE id_cat = 3');
    $cat_stmt->execute();
...
<main>
    <div class="card-container">
        <?php foreach ($produits_massif as $produit): ?>
            <div class="card">
