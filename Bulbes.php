...
// Récupérer les produits de la catégorie "Bulbes" (ID = 1)
try {
    $stmt = $pdo->prepare('SELECT * FROM produits WHERE id_cat = 1');
    $stmt->execute();
    $produits_bulbes = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Récupérer le nom de la catégorie "Bulbes" (ID = 1)
    $cat_stmt = $pdo->prepare('SELECT nom_categorie FROM categories WHERE id_cat = 1');
    $cat_stmt->execute();
...
<main>
    <div class="card-container">
        <?php foreach ($produits_bulbes as $produit): ?>
            <div class="card">
